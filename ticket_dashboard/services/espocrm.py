...
class EspoService:
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                await asyncio.gather(
                    self._fetch_entity(client, f"{self.base_url}/api/v1/Case", "Case", base_params, normalized_tickets, user_map),
                    self._fetch_entity(client, f"{self.base_url}/api/v1/Task", "Task", base_params, normalized_tickets, user_map),
                )
        ...
        try:
            ...
            if resp.status_code == HTTPStatus.OK:
                ...
                for u in users:
                    ...
                    if uid:
                        user_map[uid] = email if email else f"{u.get('userName')}@placeholder"
            cache.set(cache_key, user_map, timeout=3600)
        except httpx.HTTPError as e:
            logger.warning("Espo User Map failed: %s", e)
        return user_map

    async def _fetch_entity(self, client, url, entity_type, params, target_list, user_map):
        try:
            # Fetch first page
            max_size = 100
            request_params = {**params, "offset": 0, "maxSize": max_size}
            resp = await client.get(url, headers=self.headers, params=request_params, timeout=15.0)
            ...
            self._process_items(items, entity_type, target_list, user_map)
            ...
            if len(items) == max_size:
                ...
                if resp2.status_code == HTTPStatus.OK:
                    self._process_items(resp2.json().get("list", []), entity_type, target_list, user_map)
    ...
    def check_health(self):
        ...
        try:
            response = httpx.get(f"{self.base_url}/api/v1/App/user", headers=self.headers, timeout=3.0)
