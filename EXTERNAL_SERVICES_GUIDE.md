...
*   **Future Task Window:** By default, the dashboard only synchronizes "Open" tasks that are due within the next 30 days. This prevents the dashboard from being flooded with far-future items automatically generated by Eramba. You can adjust this window by setting the `ERAMBA_OPEN_TASK_FUTURE_WINDOW_DAYS` environment variable. "Closed" and "Pending" tasks are always synchronized regardless of their date.
