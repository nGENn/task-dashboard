...
async def test_get_user_map_pagination(op_service):
    ...
    elements_page1 = [{"id": i, "email": f"u{i}@ex.com", "login": f"u{i}"} for i in range(1, 101)]
    page1_full = {"_embedded": {"elements": elements_page1}}
    
    # Page 2 has 1 user
    page2_last = {"_embedded": {"elements": [{"id": 101, "email": "u101@ex.com", "login": "u101"}]}}
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get_call:
        ...
        async with httpx.AsyncClient() as client:
            user_map = await op_service._get_user_map(client)
...
async def test_fetch_work_packages_pagination(op_service):
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get_call:
        ...
        async with httpx.AsyncClient() as client:
            await op_service._fetch_work_packages(
                client, tasks, {}, "TestCorp"
            )
