...
class TestOpenPerspective:
    ...
    def test_open_perspective_stats(self, client):
        ...
        # Assigned tasks
        Task.objects.create(
            external_id="T4", status="open", service=service, owner="Alice"
        )
        Task.objects.create(
            external_id="T5", status="open", service=service, owner="Bob"
        )
        Task.objects.create(
            external_id="T6", status="pending", service=service, owner="Charlie"
        )
        ...
        assert stats["open"] == 2  # noqa: PLR2004
        assert stats["pending"] == 2  # noqa: PLR2004
        assert stats["unassigned"] == 1
        assert stats["total"] == 4  # noqa: PLR2004
