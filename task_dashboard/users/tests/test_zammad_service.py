...
async def test_get_user_map_pagination(zammad_service):
    # Page 1 (full)
    page1 = [{"id": i, "email": f"u{i}@ex.com", "login": f"u{i}"} for i in range(1, 251)]
    # Page 2 (last)
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get_call:
        ...
        async with httpx.AsyncClient() as client:
            user_map = await zammad_service._get_user_map(client)
            
        assert len(user_map) == 251
...
async def test_fetch_all_tasks_pagination(zammad_service):
    ...
    async def mock_get(url, **kwargs):
        ...
        if "api/v1/tickets" in url:
            if page == 1:
                resp.json = MagicMock(return_value={"tickets": page1} if page == 1 else page1)
            elif page == 2:
                resp.json = MagicMock(return_value=page2)
            else:
                resp.json = MagicMock(return_value=[])
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get_call:
        ...
        async with httpx.AsyncClient() as client:
            tasks = await zammad_service._fetch_all_tasks_async(client)
            
        assert len(tasks) == 105
        assert tasks[0]["id"] == 1
        assert tasks[104]["id"] == 105
